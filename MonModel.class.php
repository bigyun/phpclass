...
class MonModel
{
    ...
    public function __construct( $dbname = NULL, $table = NULL)
    {

        if (NULL === $dbname) {
            $this->throwError('集合不能为空！');
        }
        $this->host = '192.168.1.233';
        $this->port = '27017';
        $this->table = $table;

        $this->mongo = new \MongoDB\Driver\Manager("mongodb://".$this->host.':'.$this->port);
        //print_r($this->mongo);die;
